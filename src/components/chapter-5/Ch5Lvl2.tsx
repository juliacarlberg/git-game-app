...
export const Ch5Lvl2 = () => {
  ...
  const [answerCards, setAnswerCards] = useState<JSX.Element[]>([
    <GitCard
      ...
      title="git stash"
      icon="save"
      desc="Spara ändringar vid sidan av och gå tillbaka till tidiagre HEAD commit"
    />,
  ]);
        ...
        <div className="level-presentation">
          <div className="character-and-description">
            ...
            <p>
              ...
              cumque accusamus facere eum, necessitatibus officia neque enim
              eius ullam aperiam cum corporis velit dicta? Laboriosam fugit
              dolorum rerum quos ex!
            </p>
          </div>
        </div>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                switch (input) {
                  case "git stash":
                    newOutput += playersMove("git stash");
                    break;
                }
                setOutput(newOutput);
                setInput("");
