...
export const Ch5Lvl1 = () => {
  ...
  const [answerCards, setAnswerCards] = useState<JSX.Element[]>([
    <GitCard
      ...
      title="git pull"
      icon="arrow-down"
      desc="Hämta ändringar gjorda i ett repository"
    />,
  ]);
      ...
      <div className="game-root">
        <div className="level-presentation">
          <div className="character-and-description">
            ...
            />
            <p>
              Kommer du ihåg när du hjälpte Amanda hämta ett existerande repo?
              En kollega på företag har uppdaterat det repot med ny kod. Amanda
              behöver nu hjälp att hämta den uppdaterade koden via ett git
              kommando.
            </p>
          </div>
        </div>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                switch (input) {
                  case "git pull":
                    newOutput += playersMove("git pull");
                    break;
                }
                setOutput(newOutput);
                setInput("");
